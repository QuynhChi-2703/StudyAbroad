...
function goBack() {
    ...
    window.location.href = "DuhocMy.html"; // Đường dẫn đến trang chính
}
console.log("JavaScript file loaded successfully!");

// Điều hướng đến trang chi tiết
function openDetails(image, title, info) {
    const url = `harvard.html?=${encodeURIComponent(image)}&title=${encodeURIComponent(title)}&info=${encodeURIComponent(info)}`;
    window.location.href = url;
}
function openDetails(image, title, info) {
    const url = `MIT.html?=${encodeURIComponent(image)}&title=${encodeURIComponent(title)}&info=${encodeURIComponent(info)}`;
    window.location.href = url;
}
